...
echo "== Backup =="
cp "$MOBILE" "$MOBILE.backup-before-shelf-parser-refactor"
...
echo "== Rydder app/mobile/page.tsx =="
python3 - <<'PY'
from pathlib import Path

p = Path("app/mobile/page.tsx")
s = p.read_text()
...
def remove_type(src: str, name: str) -> str:
...
def remove_function(src: str, name: str) -> str:
  ...
  if brace == -1:
    raise SystemExit(f"Fant ikke start-brace for {name}")
  depth = 0
  end = None
...
for type_name in ["ShelfPriceCandidate", "OcrWord"]:
  s = remove_type(s, type_name)
...
for fn_name in [
  ...
  s = remove_function(s, fn_name)

# Rydd litt ekstra blanke linjer.
while "\n\n\n\n" in s:
  s = s.replace("\n\n\n\n", "\n\n\n")

p.write_text(s)
print("OK: mobile page bruker nå lib/shelf-price-parser.ts")
PY

echo "== Verifisering =="
grep -n "shelf-price-parser" "$MOBILE"
if grep -n "^function extractShelfPriceCandidates\|^type ShelfPriceCandidate\|^type OcrWord\|^function mergeShelfCandidates" "$MOBILE"; then
  echo "FEIL: parserdefinisjoner ligger fortsatt i mobile page."
  exit 1
fi

echo "OK: refaktor ferdig."
